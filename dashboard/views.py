...
from .sentiment_analysis import (
    getSentimentLabel,
    get_sentiment_barchart_bytes,
    get_top_n_words_chart_as_bytes,
)
...
def start_comment_retrieval(request):
    if request.method == "GET":
        ...
        if videoId is not None:
            ...
            return JsonResponse(
                {"error": "videoid required"}, status=status.HTTP_400_BAD_REQUEST
            )
...
@api_view(["GET"])
def get_video_topn_chart(request):
    if request.method == "GET":
        videoId = request.query_params.get("videoid", None)
        if videoId is not None:
            videoId = extract_videoid(videoId)
            barchart_bytes = get_top_n_words_chart_as_bytes(videoId, 10)
            return HttpResponse(barchart_bytes, content_type="image/png")
        else:
            return JsonResponse(
                {"error": "videoid required"}, status=status.HTTP_400_BAD_REQUEST
